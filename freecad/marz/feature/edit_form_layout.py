...
def InputFloat(*args, suffix=' mm', decimals=4, step=0.01, **kwargs):
        ...
        with ui.Row(contentsMargins=(0,0,0,0)):
            with ScrollContainer():
                ...
                with ui.Section(SectionHeader(tr('Board'))):
                    ...
                    form.fretboard_isZeroFret = ui.InputOptions(label=tr('Zero Fret'), options={"Enabled": True, "Disabled": False})
                    form.fretboard_EDO = ui.InputInt(label=tr('EDO'))
                    form.fretboard_length = InputFloat(label=tr('Board Length (0=auto)'))
...
def neck_joint(form):
    joints = dict((
        ...
        ('Extra Chunk', NeckJoint.EXTRA_CHUNK),
    ))
    ...
    with ui.Section(SectionHeader(tr('Extra Chunk'))):
        form.neck_extraChunkLength = InputFloat(label=tr('Length'))
        form.neck_extraChunkVerticalOffset = InputFloat(label=tr('Vertical Offset'))
