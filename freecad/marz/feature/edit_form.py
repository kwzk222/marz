...
class InstrumentForm(InstrumentFormBase):
    ...
    def load_from_obj(self, obj):
        ...
        # Robust access to string gauges to avoid AttributeError
        gauges = getattr(obj, 'Stringset_Gauges', getattr(obj, 'StringSet_Gauges', None))
        if gauges is not None:
            self.stringSet_gauges_f.setValue([float(f) for f in gauges])
    ...
    def save_to_obj(self, obj):
        ...
        # Robust save to string gauges
        vals = [str(f) for f in self.stringSet_gauges_f.value()]
        if hasattr(obj, 'Stringset_Gauges'):
            obj.Stringset_Gauges = vals
        elif hasattr(obj, 'StringSet_Gauges'):
            obj.StringSet_Gauges = vals
    ...
    def nut_width_calc(self, obj):
        # Robust access to string gauges
        gauges = getattr(obj, 'Stringset_Gauges', getattr(obj, 'StringSet_Gauges', None))
        if gauges is None:
            return 43.5 # Default fallback
    ...
    def update_3d(self):
        ...
        # Hide 2D objects when generating 3D as requested
