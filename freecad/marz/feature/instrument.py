        ...
        with progress_indicator("Update All..."):
            with transaction("Marz Update Parts"):
                ...
                try:
                    ...
                    if fb_obj is not None and hasattr(fbd, "frets") and fbd.frets and hasattr(fbd, "neckFrame"):
                        ...
                        if lineIntersection:
                            ...
                            if last_proj is not None:
                                # create a vertex at last_proj and find nearest point on the shape
                                v = Part.Vertex(last_proj)
                                best_d = None
                                best_pt = None
                                # check edges first
                                for ed in shp.Edges:
                                    try:
                                        d, pts_ed, _ = ed.distToShape(v)
                                        if d is None:
                                            continue
                                        if best_d is None or d < best_d:
                                            best_d = d
                                            # pts_ed may be a list/tuple — take a geometry point safely
                                            if pts_ed:
                                                candidate = pts_ed[0]
                                ...
                                if best_pt is None:
                                    for face in shp.Faces:
                                        try:
                                            d, pts_face, _ = face.distToShape(v)
                                            if d is None:
                                                continue
                                            if best_d is None or d < best_d:
                                                best_d = d
                                                if pts_face:
                                                    candidate = pts_face[0]
                                                    if isinstance(candidate, (tuple, list)) and candidate:
                                                        candidate = candidate[0]
                                                    best_pt = candidate
                                        except Exception:
                                ...
                                # if we found a geometry point, compute distances
                                if best_pt is not None and f0_proj is not None:
                                    # best_pt may be a FreeCAD vector-like object or tuple; normalize
                                    try:
                                        geo_pt = FreeCAD.Vector(best_pt.x, best_pt.y, best_pt.z)
                                    except Exception:
                                        try:
                                            geo_pt = FreeCAD.Vector(float(best_pt[0]), float(best_pt[1]), float(best_pt[2]) if len(best_pt)>2 else 0.0)
                                        except Exception:
                                            geo_pt = None

                                    if geo_pt is not None:
                                        # distance from fret0 projection (midline) to nearest geometry (this matches ruler)
                                        dist_f0_to_geo = (geo_pt.sub(f0_proj)).Length
                                        # write safely into model and UI (suppress onChanged to avoid recursion)
                                        try:
                                            self._suppress_onchange = True
                                        except Exception:
                                            pass
                                        try:
                                            try:
                                                model.fretboard.length = float(dist_f0_to_geo)
                                            except Exception:
                                                pass
                                            if hasattr(self.Object, "Fretboard_Length"):
                                                try:
                                                    self.Object.Fretboard_Length = float(dist_f0_to_geo)
                                                except Exception:
                                                    pass
                                        finally:
                                            try:
                    ...
                    # non-fatal: measurement is best-effort, ignore failures
                    pass
