...
def heel_fillet(heel, radius):
    if radius <= 0.5 or heel is None or heel.isNull():
        return heel
    ...
    try:
        ...
        faces = [f for f in heel.Faces if is_planar(f, coplanar=Z)]
        if not faces:
            return heel

        face = sorted(faces, key=lambda e: e.CenterOfMass.x)[0]
        ...
        # Silently fail if filleting is not possible due to geometry
        pass
