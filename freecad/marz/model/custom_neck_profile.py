    ...
    def _scale_shape(self, width, height, wire=True):
        ...
        try:
            ...
            for i, p in enumerate(all_pts):
                ...
                # Do not enforce exactly 0.0 if mathematical floating point
                # imprecision pushed it slightly. Instead of clamping, we let it be
                # naturally smooth, preventing sharp geometric corners which cause 
                # OpenCASCADE 'Map entry 0 is empty' faults. 
                # If strictly needed to avoid top surface protrusion, use a very tiny tolerance.
                if new_x > 1e-4:
                    new_x = 1e-4
