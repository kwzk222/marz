...
def neck_blank(inst: Instrument, fbd: FretboardData, neckd: NeckData) -> Task[NeckBase]:
    ...
    if all_profiles:
        ...
        for i in range(1, len(all_profiles)):
            ...
            # Prevent scrunched geometry and system crashes by keeping profiles sparse (min 10.0mm instead of 1.5mm to avoid paper-like artifacts near heel/headstock)
            if dist < 10.0:
                continue

            is_anchor = abs(all_profiles[i].CenterOfMass.x) < 0.5 or all_profiles[i].CenterOfMass.x > 0.1
...
def neck_blank_extra_chunk_impl(inst: Instrument, fbd: FretboardData, neckd: NeckData) -> NeckBase:
    ...
    # This part always starts at the requested start and goes towards the bridge.
    # The connection point is at fend_v.x + v_offset.
    chunk_start_x = fend_v.x + v_offset
    chunk_end_x = chunk_start_x - chunk_length

    # Z Coordinates for thin box
    # Neck surface bottom is at Z = -thickness.
    # d_offset > 0 moves it deeper (more negative Z).
    z_thin_bottom = -thickness - d_offset
    z_thin_top = z_thin_bottom + chunk_thickness

    # Clamp top to Z=0 to ensure it stays below the fretboard as requested
    z_thin_top = min(z_thin_top, 0.0)

    # Re-calculate thickness if clamped, ensuring it has at least a small positive volume
    actual_thin_thickness = max(z_thin_top - z_thin_bottom, 0.1)

    # 2. Connection Box (Vertical connector)
    # This part bridges from the neck's bottom (Z=-thickness) to the chunk's bottom (Z=z_thin_bottom).
    # Its leftmost wall MUST be at the end of the fretboard (fend_v.x).
    # It extends towards the nut (positive X).
    ...
    # To ensure connectivity when v_offset moves chunk away from the connector:
    # We must span from chunk_start_x to conn_left_x.
    x_bridge = min(chunk_start_x, chunk_end_x, conn_left_x)
    x_nut = max(chunk_start_x, chunk_end_x, conn_left_x)
